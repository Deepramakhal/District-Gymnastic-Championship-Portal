  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 px-4 max-w-6xl mx-auto">
        ...
        <ActionCard title="U8 Boys Scoring" icon="📊" onClick={() => navigateToScoringPage("U8", "MAG")} />
        <ActionCard title="U8 Girls Scoring" icon="📊" onClick={() => navigateToScoringPage("U8", "WAG")} />
      ...
      {printPopOpen && (
        <Modal onClose={() => setPrintPopOpen(false)}>
          ...
          <Select
            ...
            onChange={e => setPrintForm({ ...printForm, ageGroup: e.target.value })}
          >
            <option value="">Select Age Group</option>
            {["U6","U8","U10","U12","U14","U17","SENIOR"].map(a => (
          ...
          <input type="text" placeholder="Apparatus" value={printForm.apparatus} onChange={e => setPrintForm({ ...printForm, apparatus: e.target.value })} />
      ...
      {/* ================= ADD MODAL ================= */}
{addPlayerPopOpen && (
  <Modal onClose={() => { setAddPlayerPopOpen(false); setPlayersList([]); }}>
    ...
    {playersList.length > 0 && (
      <div className="mt-3 space-y-2 max-h-40 overflow-y-auto">
        {playersList.map((p, i) => (
          ...
          >
            <span>{p.name} — {p.ageGroup} — {p.type}</span>
            <button
    ...
    {selectedPlayer && (
      <>
        ...
        <Select
          ...
          onChange={e =>
            setEditFormData({ ...editFormData, club_name: e.target.value })
          }
        >
          {clubs.map(c => (
            <option key={c.id} value={c.name}>{c.name}</option>
...
const Modal = ({ children, onClose }) => (
  <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
    <div className="bg-white rounded-xl p-5 w-full max-w-md relative">
      <button onClick={onClose} className="absolute cursor-pointer hover:bg-red-300 p-1 rounded-2xl top-2 right-3">✕</button>
      {children}
    </div>
  </div>
);
